...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('audiobook_generator.log'),
...
class AudiobookGenerator:
    ...
    def __init__(self, language: str = "pt-BR", voice_gender: str = "FEMALE", 
    ...
    def generate_audiobook(self, input_file: str, output_name: Optional[str] = None,
                          split_chapters: bool = False, preview_mode: bool = False) -> List[str]:
        """
            ...
            if output_files:
                ...
                for output_file in output_files:
                    if output_file:
                        ...
                        print(f"{Fore.CYAN}{os.path.basename(output_file)}: {audio_info.get('duration_formatted', 'Unknown')} duration{Style.RESET_ALL}")
...
@click.option('--voice', '-v', default='FEMALE', type=click.Choice(['MALE', 'FEMALE', 'NEUTRAL']), 
              help='Voice gender (default: FEMALE)')
@click.option('--premium/--no-premium', default=True, help='Use premium Wavenet voices (default: True)')
...
def files():
    ...
    try:
        ...
        if input_files:
            ...
            print(f"{Fore.CYAN}Supported formats: {', '.join(Config.SUPPORTED_TEXT_FORMATS + Config.SUPPORTED_EBOOK_FORMATS)}{Style.RESET_ALL}")
    except Exception as e:
